...
const login = async(req:Request, res:Response): Promise<Response> => {
    ...
    try {
        ...
        res.cookie('auth', accessToken, {
          maxAge: 86400000, // 1 hr
          sameSite: 'strict',
          httpOnly: true,
          secure: process.env.JWT_SECRET_KEY as string === 'production'
        });
        const refreshToken = jwt.sign({ id: user._id }, process.env.JWT_SECRET_KEY as string, { expiresIn: '7d' });

        // Save refresh token in the user's record
...
const removeUserAccount = async(req:Request, res: Response): Promise<Response> => {
    const { id } = req.params;
    try {
